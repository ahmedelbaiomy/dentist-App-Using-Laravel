<!-- <ul class="list-group">
    ...
    <li class="list-group-item">
        @if($cat->path_icon)
        <p class="mb-0 text-center"><img style="height:60px" src="{{asset(base64_decode($cat->path_icon))}}" class="" />
        </p>
        @endif
...
<!-- <ul class="list-group list-group-horizontal-sm">
    @foreach($categories as $cat)
    <li style="cursor:pointer;" class="list-group-item" onclick="_loadDatasByCategory({{$cat->id}})">
        <p class="mb-0">
        <div class="btn-group">
            <button class="btn btn-icon btn-sm btn-outline-primary" onclick="_formCategory({{$cat->id}})"
                title="Edit">{!!\App\Library\Helpers\Helper::getSvgIconeByAction('EDIT')!!}</button>
