        ...
        <ul class="nav navbar-nav align-items-center ml-auto">
            ...
            <!-- Notifications -->
                ...
                <li class="nav-item dropdown dropdown-notification mr-25">
                    ...
                    @if(Auth::user()->user_type=='reception' && isset($doc_notifications) && (count($doc_notifications)>0 || count($pat_notifications)>0))
                    <span class="badge badge-pill badge-danger badge-up">{{count($doc_notifications) + count($pat_notifications)}}</span>
                    @elseif(Auth::user()->user_type=='admin' && isset($admin_notifications) && count($admin_notifications)>0)
                    <span class="badge badge-pill badge-danger badge-up">{{count($admin_notifications)}}</span>
                    @elseif(Auth::user()->user_type=='doctor' && isset($notifications) && count($notifications)>0)
                    ...
                    <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                        <li class="dropdown-menu-header">
                            <div class="dropdown-header d-flex">
                                ...
                                <div class="badge badge-pill badge-light-primary">{{count($doc_notifications) + count($pat_notifications)}} New</div>
                            </div>
                        </li>
                        @foreach($doc_notifications as $notific)
                            ...
                            <a class="d-flex" href="{{url('reception/view_notification/'.$notific->id)}}">
                                <div class="media d-flex align-items-start">
                                    ...
                                    <div class="media-body">
                                        <p class="media-heading"><span class="font-weight-bolder">{{$notific->message_type==10?'Appointment Booked':'Appointment Requested'}}</span></p>
                        ...
                        @endforeach
                        <li class="dropdown-menu-footer"><a class="btn btn-primary btn-block waves-effect waves-float waves-light" href="/notifications/month">Read all notifications</a></li>
                    </ul>
                    @elseif(Auth::user()->user_type=='admin' && isset($admin_notifications) && count($admin_notifications)>0)
                    <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                        <li class="dropdown-menu-header">
                            <div class="dropdown-header d-flex">
                                <h4 class="notification-title mb-0 mr-auto">Notifications</h4>
                                <div class="badge badge-pill badge-light-primary">{{count($admin_notifications)}} New</div>
                            </div>
                        </li>
                        @foreach($admin_notifications as $notific)
                        <li class="scrollable-container media-list ps">
                            <a class="d-flex" href="{{url('admin/view_notification/'.$notific->id)}}">
                                <div class="media d-flex align-items-start">
                                    <div class="media-left">
                                        <div class="avatar"></div>
                                    </div>
                                    <div class="media-body">
                                        <p class="media-heading"><span class="font-weight-bolder">Low rated reception detected.</span></p>
                                            <small class="notification-text"> {{$notific->notification}}</small>
                                    </div>
                                </div>
                        </li>
                    ...
                    <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                        ...
                        <li class="scrollable-container media-list ps">
                            <a class="d-flex" href="{{url('doctor/view_notification/'.$notific->id)}}">
                                <div class="media d-flex align-items-start">
                                    <div class="media-left">
                                        <div class="avatar"></div>
                                    </div>
                                    <div class="media-body">
                                        <p class="media-heading"><span class="font-weight-bolder">Low rate profile detected.</span></p>
                                            <small class="notification-text"> {{$notific->notification}}</small>
                                    </div>
                                </div>
                        </li>
