                                        ...
                                        <td>
                                            @if($plan->type != "completed")
                                                <button onclick="complete_func('complete_frm_{{ $plan->id }}')"  type="button" class="btn btn-primary">
                                                    <form action="{{ route('doctor.service.plan.complete', $plan->id)}}" name="complete_frm_{{ $plan->id }}" id="complete_frm_{{ $plan->id }}" method="post">
                                                        @csrf
                                                        @method('POST')
                                                        Complete
                                                    </form>    
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="form-control-wrap">
                                <input type="text" id="from" name="from" value = "{{ Auth::user()->email }}" class="form-control form-control-lg" readonly>
                                <input type="hidden" id="from_id" name="from_id" value = "{{ Auth::user()->id }}" class="form-control form-control-lg" readonly>
                            </div>
                        </div>
                    </div>
...
var plan_arr = Array();
    $('#add_invoice_modal').on('show.bs.modal', function(e) {
        ...
        $('#plan_tbody [type="checkbox"]').each(function(i, chk) {
            if (chk.checked) {
                ...
                //console.log("Checked!", i, chk.value);
            }
        });
    ...
    $("#invoice_save_btn").on('click', function(e) {
        ...
        $.ajax({
            url: '{{route("doctor.invoice.store")}}',
            type:"POST",
            data:{
                code: $("#code").text(),
                from: $("#from_id").val(),
